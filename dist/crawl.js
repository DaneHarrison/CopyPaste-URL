...
import { createAssetPath, delay, ensureDir, isSameRegistrableDomain, normalizeUrl, slugifyUrl, writeJson, writeText } from "./utils.js";
function inferFontExtension(contentType, url) {
...
function isAllowedByRobots(rules, url) {
    ...
    const disallowed = rules.disallow.find((rule) => pathToCheck.startsWith(rule.replace(/\*/g, "")));
    if (!disallowed)
        return true;
    const allowed = rules.allow.find((rule) => pathToCheck.startsWith(rule.replace(/\*/g, "")));
    return Boolean(allowed);
}
function addToSitemap(root, record) {
    ...
    segments.forEach((segment, index) => {
        ...
        if (!current.children.has(key)) {
            current.children.set(key, {
                url: new URL(segments.slice(0, index + 1).join("/") || "/", `${url.protocol}//${url.host}`).toString(),
...
function renderSitemap(node, depth = 0, lines = []) {
...
async function downloadAsset(assetUrl, targetDir, relativeRoot, cache) {
    try {
        ...
        try {
            ...
            const links = await page.evaluate((selectors) => {
                ...
                return Array.from(document.querySelectorAll("a[href]"))
                    ...
                    const visibility = styles.display === "none" ||
            ...
            for (const asset of assetUrls) {
                ...
                if (!resolved)
                    continue;
                const saved = await downloadAsset(resolved, pageAssetDir, config.outputDir, assetCache);
                if (saved) {
